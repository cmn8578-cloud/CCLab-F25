...
class Cloud {
  ...
  constructor(x, y, s) {
    ...
    this.y = y;
    this.y0 = y;
    //have this variable to store the intitial value when moving the clouds => constant values
    this.s = s;
    this.speedX = map(this.s, 30, 100, 3, 0.5); //small faster, big slower 
    this.h = random(100);
  }
  ...
  move() {
    ...
    // this.x = this.x0 + 50 * cos(frameCount * 0.1); // if using this.x -> not constant 
    // this.y = this.y0 + 50 * sin(frameCount * 0.1);
    // this.s = map(sin(frameCount * 0.05), -1, 1, this.s0 * 0.7, this.s0, 100);

  }

  moveback() {
    if (this.x > width + 2 * this.s) {
      // this.x = random(-width, -width * 0.1) //reset the value everytime we put back 
      this.x += //reset the value everytime we put back 
